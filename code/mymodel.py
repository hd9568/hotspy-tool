...
class BinaryClassificationMLP(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.relu(self.fc1(x))
        # 通过第二个全连接层后再次使用ReLU激活函数
        x = F.relu(self.fc2(x))
        # 通过输出层
        x = self.output(x)
        # 使用softmax激活函数将输出转换为概率
        x = torch.softmax(x,dim=0)
        return x

# 假设输入向量的大小为10，隐藏层大小为50
# input_size = 10
# hidden_size = 50

# # 实例化模型
# model = BinaryClassificationMLP(input_size, hidden_size)

# # 打印模型结构
# print(model)
# 示例张量
# tensor = torch.tensor([0.0036], device='cuda:0')

# # 将张量移动到CPU上，并提取其值
# value = float(tensor.cpu().item())

# print(value)  # 输出: 0.0036，这是一个小数
